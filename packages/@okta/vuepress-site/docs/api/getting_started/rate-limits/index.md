...
The most common APIs are covered here; this is not an exhaustive list of every single API within Okta. If you need more information about a particular API that is not covered on this page, please contact Okta Support. Unless otherwise stated, a shared rate limit applies to all APIs that share the listed path as a prefix to their URI. In cases where multiple paths are a prefix, the longest prefix will apply. Remember, the most specific rate limit will always be the one that applies.
